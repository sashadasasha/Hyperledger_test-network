...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	...
	CC_SRC_PATH="../chaincode/fabcar/java/build/install/oddeven"
...
packageChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode package oddeven.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label oddeven_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode install oddeven.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n oddeven -c '{"Args":["queryAllNumbers"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
